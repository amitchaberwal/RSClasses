                                ...
                                child: InkWell(
                                  onTap: (){
                                    Navigator.of(context)
                                        .push(MaterialPageRoute(builder: (BuildContext context) => getMessages(batchID: mdocc.id,)));
                                  },
                                  child: Container(
          ...
          new IconButton(
            ...
            onPressed: (){
              showDialog(context: context,
                  builder: (BuildContext context){
                    return CreateMessage(batchId: widget.batchID,);
                  }
              );
            },
          )
                        ...
                        child: Stack(
                            children: [
                              ...
                              Align(
                                ...
                                child: IconButton(
                                  onPressed: () async {
                                    await FirebaseFirestore.instance.collection('Batch').doc(widget.batchID).collection('ClassMessage').doc(mdoc.id).delete();
                                  },
...
class CreateMessage extends StatefulWidget {
  final String batchId;
  const CreateMessage({Key key, this.batchId}) : super(key: key);
...
class _CreateMessageState extends State<CreateMessage> {
  ...
  Future uploadData(BuildContext context) async {
    ...
    if(mtitle == null || mdesc == null){
      ...
      await pr.show();
      await FirebaseFirestore.instance.collection('Batch').doc(widget.batchId).collection('ClassMessage').doc().set({
        'Title': mtitle,
        'Message': mdesc,
        'Link': mlink,
        'PostDate':sdate
